(function() {
  var CA, CAattrs, CAextensions, Forge, Promise, ServerAttrs, ServerExtensions, _, asterisksRe, fs, generateKeyPairAsync, ipAddressRe, os, path, pki;
  ...
  CA = (function() {
    ...
    CA.prototype.removeAll = function() {
      return fs.removeAsync(this.baseCAFolder).catchReturn({
        code: "ENOENT"
      });
    };
    ...
    CA.prototype.generateCA = function() {
      return generateKeyPairAsync({
        ...
        return function(keys) {
          ...
          _this.CAcert = cert;
          _this.CAkeys = keys;
          return Promise.all([fs.outputFileAsync(path.join(_this.certsFolder, "ca.pem"), pki.certificateToPem(cert)), fs.outputFileAsync(path.join(_this.keysFolder, "ca.private.key"), pki.privateKeyToPem(keys.privateKey)), fs.outputFileAsync(path.join(_this.keysFolder, "ca.public.key"), pki.publicKeyToPem(keys.publicKey))]);
        };
      })(this));
    };
    ...
    CA.prototype.generateServerCertificateKeys = function(hosts) {
      ...
      return Promise.all([fs.outputFileAsync(path.join(this.certsFolder, dest + ".pem"), certPem), fs.outputFileAsync(path.join(this.keysFolder, dest + ".key"), keyPrivatePem), fs.outputFileAsync(path.join(this.keysFolder, dest + ".public.key"), keyPublicPem)])["return"]([certPem, keyPrivatePem]);
    };
    ...
    CA.create = function(caFolder) {
      var ca;
      ca = new CA(caFolder);
      return fs.statAsync(path.join(ca.certsFolder, "ca.pem")).bind(ca).then(ca.loadCA)["catch"](ca.generateCA)["return"](ca);
    };
