(function() {
  var DEFAULT_DOMAIN_NAME, Promise, Request, Server, Socket, _, agent, allowDestroy, appData, blacklist, buffers, check, compression, concatStream, cors, cwd, debug, ensureUrl, errors, evilDns, exphbs, express, fileServer, fullyQualifiedRe, headersUtil, http, httpProxy, httpsProxy, isHtml, isResponseHtml, la, logger, notSSE, origin, setProxiedUrl, statusCode, stream, uri, url;
  ...
  Server = (function() {
    ...
    Server.prototype.open = function(config, project, onWarning) {
      if (config == null) {
        config = {};
      }
    ...
    Server.prototype._onResolveUrl = function(urlStr, headers, automationRequest, options) {
      ...
      return this._urlResolver = p = new Promise((function(_this) {
        return function(resolve, reject, onCancel) {
          var newUrl, obj, onReqError, onReqStreamReady, redirects, restorePreviousState, urlFile;
          onCancel(function() {
          ...
          if (obj = buffers.getByOriginalUrl(urlStr)) {
            ...
            return runPhase(function() {
              return resolve(Promise.map(obj.details.cookies, _.partial(automationRequest, 'set:cookie'))["return"](obj.details));
            });
          }
          redirects = [];
          newUrl = null;
