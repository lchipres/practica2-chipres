(function() {
  var Automation, EE, Project, Promise, R, Reporter, Server, Watchers, _, api, browsers, cache, check, commitInfo, config, cwd, debug, errors, fs, keys, la, localCwd, logger, multipleForwardSlashesRe, path, plugins, preprocessor, savedState, scaffold, scaffoldDebug, settings, specsUtil, user,
  ...
  Project = (function(superClass) {
    ...
    Project.prototype.open = function(options) {
      ...
      return this.getConfig(options).tap((function(_this) {
        ...
        return function(cfg) {
          return _this.server.open(cfg, _this, options.onWarning).spread(function(port, warning) {
            if (!cfg.port) {
              cfg.port = port;
    ...
    Project.prototype.watchSettings = function(onSettingsChanged, options) {
      var obj;
    ...
    Project.prototype.getProjectId = function() {
      return this.verifyExistence().then((function(_this) {
        return function() {
          return settings.read(_this.projectRoot, _this.options);
        };
      })(this)).then((function(_this) {
        return function(readSettings) {
          ...
          return errors["throw"]("NO_PROJECT_ID", settings.configFile(_this.options), _this.projectRoot);
        };
      })(this));
    };
    ...
    Project.getProjectStatuses = function(clientProjects) {
      ...
      return user.ensureAuthToken().then(function(authToken) {
        debug("got auth token: %o", {
          authToken: keys.hide(authToken)
        });
    ...
    Project.add = function(path, options) {
    ...
    Project.ensureExists = function(path, options) {
